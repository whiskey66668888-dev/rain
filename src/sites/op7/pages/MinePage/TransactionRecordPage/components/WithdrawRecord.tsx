...
export const WithdrawRecord = ({
  ...
  const list: TWithdrawRecordItem[] = useMemo(() => {
    if (!data) return [];
    // data.pages 是 react-query 返回的分页数组，这里做一次显式断言
    const pages = (data as { pages: { list: TWithdrawRecordItem[] }[] }).pages;
